...
`Task` 컴포넌트를 만들기 시작할 때 먼저 위에서 살펴본 여러 유형의 task에 해당하는 테스트 상태(state)를 작성합니다. 그런 다음 모의 데이터를 사용하여 독립적 환경에서 컴포넌트를 구축하기 위해 Storybook을 사용합니다. 각각의 state에 따라 컴포넌트의 모습을 수동으로 테스트합니다.
...
스토리를 만들 때, 우리는 기본 `task` 인수(arg)를 사용하여 컴포넌트가 예상하는 task의 형태를 구축합니다. 이는 일반적으로 실제 데이터가 어떻게 보이는지 모델링 합니다. 다시 말하지만, `export`하는 것은 차후 스토리에서 이를 재사용 할 수 있도록 해줍니다.
